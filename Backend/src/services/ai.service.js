...
// console.log(process.env.GOOGLE_GENAI_APIs_KEY)
const ai = new GoogleGenAI({
    apiKey:process.env.GOOGLE_GENAI_APIs_KEY,
});


const interviewReportSchema= z.object({
        matchscore:z.string().describe("the match score between candidate profile matching with  job describe in btween 0 to 100"),
        ...
        })).describe("Techincal questions that can be asked in interview along with their intention and how to answer them "),
        behavioralQuestion:z.array(z.object({
            question:z.string().describe("Behavourial question that can be ask in interview "),
            intention:z.string().describe("Intention of Interviwer behind asking this question"),
            answer:z.string().describe("how to answer this question, what points to cover, what approach to take etc.")
        })).describe("How to answer this question, what points to cover , what approach to take etc"),
        skillGap:z.array(z.object({
            skill: z.string().describe("The skill where the candidate is lacking"),
            severity:z.enum(["low","medium","high"]).describe("the severity of  skill gap")
        })).describe("List ok skill gaps in the candidate profile along with their severity"),
        preparation: z.array(z.object({
            day:z.number().describe("the number of day in preparation plan starting from 1"),
            focus:z.string().describe("The main focus of this day in the preparation plan eg. data structure and algorithm, system design, behavioural question etc."),
            task :z.array(z.string().describe("the task to be done on this day for preparation eg. solve 2 questions on linked list, read 2 system design case study, prepare answer for 5 behavioural question etc.")),
...
const generateInterviewReport= async(resume,jobDescription,selfDescription)=>{
    // console.log(selfDescription,jobDescription,resume)
    // const response =await ai.models.generateContent({
    //     model:"gemini-2.0-flash",
    //     // model:"gemini-3.1-flash-lite-preview",
    //     // model:"gemini-3-flash-preview",
    //     contents : `generate an interview report for and candidate with the following details:
    //                 understand the given data and provide the response in  structure  format 
    //                     resume : ${resume},
    //                     Self Description:${selfDescription},
    //                     Job Description: ${jobDescription} `,
    //     config: {
